...
type CronJobRepository interface {
	...
	UpdateJobRunTimes(ctx context.Context, jobID int, lastRunAt, nextRunAt time.Time) error
	...
	MarkFailure(ctx context.Context, jobID int, errMsg string) error

	// Activate Job
	Activate(ctx context.Context, jobID int) error

	// Deactivate Job
	DeActivate(ctx context.Context, jobID int) error
}
